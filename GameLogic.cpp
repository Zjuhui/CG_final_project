...
//new
Bullet::Bullet(glm::vec3 pos, glm::vec3 dir, float time, unsigned int id, glm::vec3 color, float speed) : bornPosition(pos), bornDirection(dir), bornTime(time) {
...
Player::Player(glm::vec3 pos, glm::vec3 color, unsigned int teamid, glm::vec3 dir, float accelaration) {
	this->position = pos;
...
void Player::update(float time) {
    ...
    if (speed < 60.0f && speed >= 0.0f) {
        speed += accelaration * moveTime;
        if (speed >= 60.0f) {
            speed = 60.0f;
        }
    }
...
void GameLogic::checkBullet(Bullet* bullet, float time) {
    ...
    for (auto& player : Player::playerQueue) {
        if (PhysicalEngine::intersect(*player, *bullet)) {
            ...
            player->halfLength = 1.0f; //+ 0.4f * (float)player->hp / 50.0f;
            bullet->alive = false;
        }
    }
}
