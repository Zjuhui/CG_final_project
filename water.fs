...
void main(void) {
    ...
    //vec3 result = (global + (1.0 - shadow) * (diffuse + specular_a)) * resultColor;

    //out_Color = vec4(result, 1);
    out_Color = mix(out_Color, vec4(0.34, 0.70, 0.875, 1.0), 0.2) + vec4(specularHighlight, 0); //+ vec4(result, 0);
}
